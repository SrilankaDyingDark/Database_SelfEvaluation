...
声明：本学期课程学习过程中我使用了onenote软件进行记录2-9周的课堂笔记，使用了Typora进行记录10-15周的课堂笔记（详情见[Notes文件夹](https://github.com/SrilankaDyingDark/Database_SelfEvaluation/tree/main/Notes)）
...
- **所思所想**：学习多表连接时，我了解到USING 强制指定属性的写法可能掩盖潜在的结构冲突。例如学生表和课程表都有 dept_name 时，若盲目用 NATURAL JOIN 会意外引入非预期的系所匹配条件，这验证了“连接的本质是集合运算，而非单纯的字段对齐。
...
- **所思所想**：发现直接使用 `timestamp` 会丢失时区信息。通过 `current_time at time zone 'Asia/Shanghai'` 实验才理解"时间数据本质是带上下文的量值"。后来用 `interval '2 hours'` 处理跨时区数据同步时，突然意识到这和CPU时钟周期转换是相同概念——都是基准单位的换算。

### 第十一~十五周

- **学习内容**：设计、概念、理论
- **[学习笔记](https://github.com/SrilankaDyingDark/Database_SelfEvaluation/blob/main/Notes/week10_design.md)**
- **所思所想**：这一部分理论学习对我来说难度很大，关于范式在上课时我没有太理解，课后在b站找了几个视频来学习，也还是有些似懂非懂。在论及是否存储教师年龄时，考虑到"既然能通过生日计算就不该存储"，但结合派生属性原则，最终仍添加了`age`字段——因为业务需要高频按年龄区间筛选，而`DATEDIFF`计算会全表扫描。这让我想起范式理论："好设计"终究要服务于具体场景，就像BCNF和3NF没有绝对优劣。

### 分数详解

在本学期的前十一周我的学习比较认真，每一堂课都仔细做了笔记，虽然学了忘忘了学，但是总的来看还是基本达到我自身的要求；但在后期讲到理论时，由于理解上的困难我的学习效果并不是很理想，因此扣除4分。小计`16分`
...
根据下面TODO的完成情况并考虑到过程中的投入和付出，综合自评小计`17分`
